...
impl Clone for Value<'_> {
	...
	fn clone(&self) -> Self {
		if !self.is_copy() {
			// SAFETY: All of `Text`, `Ast`, and `Custom`s have their first field as `AtomicUsize`s.
			unsafe {
...
impl Drop for Value<'_> {
	...
	fn drop(&mut self) {
		...
		let rc = unsafe { self.refcount() }.fetch_sub(1, Ordering::Release);
...
impl<'env> Value<'env> {
	...
	// SAFETY: must be a constant or a number, or cloned beforhenad
	#[inline]
