...
use crate::{Environment, Error, Number, Result, Text, Value};
...
impl Function {
    ...
    pub fn register(name: char, arity: usize, func: FuncPtr) {
        FUNCTIONS
            .lock()
            .unwrap()
            .insert(name, Self { name, arity, func });
    }
}
...
pub static NOOP_FUNCTION: Function = Function {
    name: ':',
    arity: 1,
    func: noop,
};
...
pub static BLOCK_FUNCTION: Function = Function {
    name: 'B',
    arity: 1,
    func: block,
};
...
pub fn ascii(args: &[Value], env: &mut Environment<'_, '_, '_>) -> Result<Value> {
    match args[0].run(env)? {
        ...
        other => error!(Error::InvalidOperand {
            func: 'A',
            operand: other.typename()
        }),
    }
}
...
pub fn divide(args: &[Value], env: &mut Environment<'_, '_, '_>) -> Result<Value> {
    match args[0].run(env)? {
        Value::Number(lhs) => lhs
            .checked_div(args[1].run(env)?.to_number()?)
            .map(Value::from)
...
pub fn modulo(args: &[Value], env: &mut Environment<'_, '_, '_>) -> Result<Value> {
    match args[0].run(env)? {
        Value::Number(lhs) => lhs
            .checked_rem(args[1].run(env)?.to_number()?)
            .map(Value::from)
...
pub fn power(args: &[Value], env: &mut Environment<'_, '_, '_>) -> Result<Value> {
    let base = match args[0].run(env)? {
        Value::Number(lhs) => lhs,
        other => {
            return error!(Error::InvalidOperand {
                func: '^',
                operand: other.typename()
            })
        }
    ...
    Ok(Value::Number(if base == 1 {
        ...
        match exponent {
            1 => base,
            0 => 1,
            _ if base == 0 && exponent < 0 => {
                return error!(Error::DivisionByZero { kind: "power" })
            }
            _ if exponent < 0 => 0,
...
pub fn assign(args: &[Value], env: &mut Environment<'_, '_, '_>) -> Result<Value> {
    let variable = if let Value::Variable(ref variable) = args[0] {
        variable.clone()
