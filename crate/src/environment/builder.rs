...
fn system_err(_: &str) -> Result<Text, RuntimeError> {
	Err(RuntimeError::Custom(Box::new(NotEnabled)))
}

fn system_normal(cmd: &str) -> Result<Text, RuntimeError> {
	let output =
...
static mut SYSTEM_ERR: fn(&str) -> Result<Text, RuntimeError> = system_err;
static mut SYSTEM_NORMAL: fn(&str) -> Result<Text, RuntimeError> = system_normal;
