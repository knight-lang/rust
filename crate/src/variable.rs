...
#[derive(Clone)]
pub struct Variable<'f>(RefCount<(SharedText, Mutable<Option<Value<'f>>>)>);
...
impl Borrow<Text> for Variable<'_> {
	#[inline]
	fn borrow(&self) -> &Text {
		self.name()
	}
